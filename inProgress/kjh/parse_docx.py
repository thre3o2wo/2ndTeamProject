"""
법령 DOCX 파일을 조문 단위로 분리하여 CSV로 저장하는 스크립트
"""
...
def parse_law_docx(
    ...
    """
    법령 DOCX 파일을 파싱하여 조문 단위로 분리
    
    Args:
        file_path: DOCX 파일 경로
        law_name: 법령명 (예: "주택임대차보호법")
        law_type: 법령 유형 (예: "특별법", "시행령", "시행규칙")
        priority: 우선순위 (1: 특별법, 2: 시행령, 3: 시행규칙)
        effective_date: 시행일 (YYYY-MM-DD)
    
    Returns:
        pd.DataFrame: 조문 정보를 담은 데이터프레임
    """
    try:
...
if __name__ == "__main__":
    ...
    if not docx_files:
        ...
        print(f"   {LAW_RAW_DIR}")
        print(f"\n✅ 현재 디렉토리 확인:")
        print(f"   존재 여부: {LAW_RAW_DIR.exists()}")
        if LAW_RAW_DIR.exists():
            all_files = list(LAW_RAW_DIR.glob("*"))
            if all_files:
        ...
        print(f"\n📝 예시 파일명:")
        print(f"   - 주택임대차보호법(법률)(제21065호)(20260102).docx")
        print(f"   - 주택임대차보호법 시행령(대통령령)(제35947호)(20260102).docx")
        print(f"   - 민법(법률)(제20432호)(20260101)-간략.docx")
        ...
        sample_df = pd.DataFrame([
            {
                "id": str(uuid.uuid4()),
                "content": "임대인이 임차인에게 임대차보증금의 반환을 지체한 경우에는...",
                "law_family": "주택임대차",
                "law_name": "주택임대차보호법",
                "law_type": "특별법",
                "article": "제3조",
        ...
        sample_df.to_csv(LAW_CSV_PATH, index=False, encoding="utf-8-sig")
        print(f"✅ 샘플 CSV 저장: {LAW_CSV_PATH}")
        ...
        for docx_file in docx_files:
            ...
            # 파일명에서 법령 정보 추출 (간단한 휴리스틱)
            filename = docx_file.stem
            
            if "주택임대차보호법" in filename and "시행령" not in filename and "규칙" not in filename:
                law_name = "주택임대차보호법"
                law_type = "특별법"
                priority = 1
                ...
                law_type = "시행령"
                priority = 2
            elif "규칙" in filename or "확정일자" in filename:
                law_name = "확정일자 규칙"
                law_type = "시행규칙"
                priority = 3
            elif "민법" in filename:
