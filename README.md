...
| **Fusion** | RRF | Dense + BM25-Text + BM25-Title 3채널 랭크 결합 |
...
- **폴백 체인**: pdfplumber → PyMuPDF → EasyOCR → Tesseract
...
class RAGConfig:
    ...
    # 검색 설정
    embedding_backend: str = "upstage"          # 임베딩 백엔드
    candidate_k: int = 60                       # Dense 검색 후보 수
    final_k: int = 8                            # 최종 반환 문서 수
    
    # BM25 설정
    enable_bm25: bool = True                    # BM25 활성화
    bm25_k1: float = 1.5                        # BM25 k1 파라미터
    bm25_b: float = 0.75                        # BM25 b 파라미터
    
    # Rerank 설정
    enable_rerank: bool = True                  # Cohere rerank 활성화
    rerank_model: str = "rerank-multilingual-v3.0"
    rerank_max_docs: int = 80                   # 리랭크 최대 입력 문서 수
```
...
#### OCR 엔진 우선순위
1. **텍스트 PDF**: pdfplumber (직접 추출)
2. **이미지 PDF/이미지**: EasyOCR (한국어 최적화) → Tesseract (폴백)
...
config = RAGConfig(
    ...
    # 검색 튜닝
    candidate_k=60,                      # Dense 검색 후보 수
    final_k=8,                           # 최종 참조 문서 수
...
```python
config = RAGConfig(
    bm25_k1=1.5,      # 용어 빈도 영향 (높을수록 빈도 중요)
    bm25_b=0.75,      # 문서 길이 정규화 (0~1, 높을수록 길이 영향↑)
)
```
